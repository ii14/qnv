...
GridRenderer::GridRenderer(QQuickItem* parent)
...
void GridRenderer::setGrid(Grid* grid)
{
    ...
    mGrid = grid;
    if (grid != nullptr) {
        connect(grid, &Grid::hiddenChanged, this, [this]() {
            setVisible(not mGrid->mHidden);
        });
    }
    emit gridChanged();
}
...
void GridRenderer::paint(QPainter* p)
{
    ...
    auto setHl = [&](uint32_t id) {
        ...
        bg = it != session->mHls.constEnd() && it->mBgColor != -1
            ? QColor::fromRgb(it->mBgColor) : session->mBgColor;
        auto nfg = it != session->mHls.constEnd() && it->mFgColor != -1
            ? QColor::fromRgb(it->mFgColor) : session->mFgColor;
        if (fg != nfg) {
            fg = nfg;
            pen.setColor(fg);
            p->setPen(pen);
