...
int main(int argc, char** argv)
{
    ...
    QObject::connect(nvim, &NvimProcess::gridDestroy, [&](Integer id) {
        gridList->remove(id);
    });
    ...
    QObject::connect(nvim, &NvimProcess::gridScroll, [&](Integer id, Integer top, Integer bot, Integer left, Integer right, Integer rows, Integer cols) {
        if (Grid* grid = gridList->get(id))
            grid->onGridScroll(top, bot, left, right, rows, cols);
    });

    QObject::connect(nvim, &NvimProcess::winPos, [&](Integer id, Window win, Integer startRow, Integer startCol, Integer width, Integer height) {
    ...
    QObject::connect(nvim, &NvimProcess::msgSetPos, [&](Integer id, Integer row, Boolean scrolled, const String& sepChar) {
