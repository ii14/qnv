...
uint32_t utf8toUcs(std::string_view str)
{
  ...
  if (d[0] < 0x80) {
    ...
    if ((d[1] & 0xC0) != 0x80)
      return kInvalidCodepoint;
    const uint32_t r = (static_cast<uint32_t>(d[0] & 0x1F) << 6) | (d[1] & 0x3F);
    ...
    if (((d[1] & 0xC0) != 0x80) || ((d[2] & 0xC0) != 0x80))
      return kInvalidCodepoint;
    const uint32_t r = (static_cast<uint32_t>(d[0] & 0x0F) << 12)
        | (static_cast<uint32_t>(d[1] & 0x3F) << 6) | (d[2] & 0x3F);
    ...
    if (((d[1] & 0xC0) != 0x80) || ((d[2] & 0xC0) != 0x80) || ((d[3] & 0xC0) != 0x80))
      return kInvalidCodepoint;
    const uint32_t r = (static_cast<uint32_t>(d[0] & 0x07) << 18)
        | (static_cast<uint32_t>(d[1] & 0x3F) << 12) | (static_cast<uint32_t>(d[2] & 0x3F) << 6)
        | (d[3] & 0x3F);
